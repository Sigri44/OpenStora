#!/bin/sh
# Credit to : https://github.com/streetster/stora
...
# 0.05 - Ignore Github certificate check
#
######################################################
...
replaceSSHd() {
    ...
    # download
    wget -O $temp/sshd.tar.gz $SSHD_LOCATION --no-check-certificate
...
installIPKG() {
    ...
    # download
    wget -O $temp/ipkg.ipk $IPKG_LOCATION --no-check-certificate
...
installNFS() {
    ...
    then
        ...
        # download
        wget -O $temp/nfs.tar.gz $NFS_LOCATION --no-check-certificate
...
# check if we have IPKG (again)
    ...
    # setup nzbget daemon
    wget -O $temp/nzbgetd $NZBGETD_LOCATION --no-check-certificate
...
installNZBgetServer() {
    ...
    # download 
    wget -O $temp/nzbgetweb.zip $NZBGETWEB_LOCATION --no-check-certificate
    wget -O $temp/xmlrpc.tar.gz $RPCLIB_LOCATION --no-check-certificate
...
wget $TRANSMISSIOND_LOCATION -O $temp/transmission-daemon.default --no-check-certificate
